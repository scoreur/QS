...
struct Chunk {
	...
	Chunk() :
			type { 0, 0, 0, 0 }, length { 0, 0, 0, 0 }, content(0) {
	}                //for writing
...
struct ControlChunk {
	...
	ControlChunk(u32 delta, char* &in) :
			deltaTime(delta), type(*(in++)), content(0) {
	...
	void display() {            //debug
		std::cout << std::dec << deltaTime << ':' << "type: " << std::hex
				<< (u16) (u8) type << ';';
...
struct MidiEvent {
	...
	void display() {            //debug
		std::cout << std::dec << deltaTime << ':' << "type: " << std::hex
				<< (u16) (u8) type << ';' << "param1: " << std::hex
				<< (u16) param1 << ';' << "param2: " << std::hex << (u16) param2
				<< '\n';
		std::cout << std::endl;
	}
};
...
public:
	...
	enum Format {
		single = 0, multi_sync, multi_independent
	} fmt;
